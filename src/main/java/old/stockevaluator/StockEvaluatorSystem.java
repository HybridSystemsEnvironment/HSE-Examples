...
public class StockEvaluatorSystem extends HybridSystem<StockEvaluatorState>
{
	...
	public void G(StockEvaluatorState x, StockEvaluatorState x_plus)
	{
		...
		for (int i = 0; i < x_plus.stocks.size(); i++)
		{
			...
			x_plus.stocks.get(i).stockValue = x_plus.stocks.get(i).getStockPrice(); // compute stock value
	...
	/*
	 * Creates all of the stock states, which aren't already included in the
	 * state, that will be monitored by this evaluator
	 */
		...
		for (String stockIndex : stockIndices)
		{
			if (stockIndex != null) // make sure index is non-null
			{
				try // attempt to create a stock state for the current index
				{
					...
					new ChartRequestBuilder().withChartRange(ChartRange.ONE_DAY).withSymbol(stockIndex).build());
	...
	private SimpleRegression getStockHistory(StockState state)
	{
		...
		SimpleRegression stockHistory = state.getStockHistory(historyInterval); // get history
																				// regression
		return stockHistory;
	}
	...
	private SimpleRegression getStockLogHistory(StockState state)
	{
		...
		SimpleRegression stockHistory = state.getStockLogHistory(historyInterval); // get history
																					// regression
		return stockHistory;
	}
