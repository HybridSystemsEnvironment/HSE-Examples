...
public class Dp implements JumpSet<InverterState> {
	...
	public boolean D(InverterState x) {
		...
		if (x.p == 1.0) {
			...
			{
				if (x.iL * x.vC * params.FIc <= 0.0
						|| (x.iL * x.vC * params.FIc >= 0.0 && Math.abs(x.iL) >= params.eps)) {
					return true;
				} else if (x.iL * x.vC * params.FIc >= 0 && Math.abs(x.iL) <= params.eps && x.q != 0.0) // switch q for
																										// z in M
				{
					return true;
				}
